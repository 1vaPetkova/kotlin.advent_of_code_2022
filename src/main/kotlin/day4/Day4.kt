...
private fun getElfRange(range: List<String>): List<Int> {
    val list = mutableListOf<Int>()
    for (i in range[0].toInt() until range[1].toInt() + 1) { list.add(i) }
    return list
}

private fun readInput(): List<Pair<List<Int>, List<Int>>> {
    return File(PATH).readLines().map { it.split(",").toList() }.map {
        val first = getElfRange(it[0].split("-").toList())
        val second = getElfRange(it[1].split("-").toList())
        Pair(first, second)
    }.toList()
}